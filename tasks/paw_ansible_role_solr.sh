...
if [ -n "$PT__installdir" ]; then
  ANSIBLE_DIR="$PT__installdir/lib/puppet_x/ansible_modules/ansible_role_solr"
else
  # Fallback to /opt/puppetlabs/puppet/cache/lib/puppet_x/ansible_modules
  ANSIBLE_DIR="/opt/puppetlabs/puppet/cache/lib/puppet_x/ansible_modules/ansible_role_solr"
fi
...
if [ -d "$ANSIBLE_DIR/roles" ] && [ -f "$ANSIBLE_DIR/roles/paw_ansible_role_solr/playbook.yml" ]; then
  # Collection structure
  PLAYBOOK_PATH="$ANSIBLE_DIR/roles/paw_ansible_role_solr/playbook.yml"
  PLAYBOOK_DIR="$ANSIBLE_DIR/roles/paw_ansible_role_solr"
  ...
  echo "{\"_error\": {\"msg\": \"playbook.yml not found in $ANSIBLE_DIR or $ANSIBLE_DIR/roles/paw_ansible_role_solr\", \"kind\": \"puppet-ansible-converter/playbook-not-found\"}}"
  exit 1
fi
...
if [ -n "$PT_solr_workspace" ]; then
  ...
  EXTRA_VARS="$EXTRA_VARS\"solr_workspace\": \"$PT_solr_workspace\""
fi
if [ -n "$PT_solr_create_user" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
...
if [ -n "$PT_solr_group" ]; then
  ...
  EXTRA_VARS="$EXTRA_VARS\"solr_group\": \"$PT_solr_group\""
fi
if [ -n "$PT_solr_version" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"solr_version\": \"$PT_solr_version\""
fi
if [ -n "$PT_solr_mirror" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"solr_mirror\": \"$PT_solr_mirror\""
fi
if [ -n "$PT_solr_remove_cruft" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"solr_remove_cruft\": \"$PT_solr_remove_cruft\""
fi
if [ -n "$PT_solr_service_manage" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
...
if [ -n "$PT_solr_service_state" ]; then
  ...
  EXTRA_VARS="$EXTRA_VARS\"solr_service_state\": \"$PT_solr_service_state\""
fi
if [ -n "$PT_solr_install_dir" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
...
if [ -n "$PT_solr_timezone" ]; then
  ...
  EXTRA_VARS="$EXTRA_VARS\"solr_timezone\": \"$PT_solr_timezone\""
fi
if [ -n "$PT_solr_opts" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"solr_opts\": \"$PT_solr_opts\""
fi
if [ -n "$PT_solr_cores" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"solr_cores\": \"$PT_solr_cores\""
fi
if [ -n "$PT_solr_default_core_path" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"solr_default_core_path\": \"$PT_solr_default_core_path\""
fi
if [ -n "$PT_solr_config_file" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"solr_config_file\": \"$PT_solr_config_file\""
fi
if [ -n "$PT_solr_restart_handler_enabled" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"solr_restart_handler_enabled\": \"$PT_solr_restart_handler_enabled\""
fi
EXTRA_VARS="$EXTRA_VARS}"
...
if [ $EXIT_CODE -eq 0 ]; then
  echo '{"status": "success", "role": "ansible_role_solr"}'
else
  echo "{\"status\": \"failed\", \"role\": \"ansible_role_solr\", \"exit_code\": $EXIT_CODE}"
fi
